...
import { Star, Tv, Clock, Building2, Shield, Heart, ListPlus, Share2 } from "lucide-react";
...
function AnimePageContent() {
  ...
  if (!id) return <div className="p-10 text-center">Anime seçilmedi. <Link href="/" className="text-[#6750A4] underline">Keşfet</Link></div>;
  if (loading) return <div className="h-[60vh] rounded-[28px] bg-[#F3EDF7] dark:bg-[#211F26] animate-pulse"/>;
  if (!data) return <div className="p-10 text-center">Anime bulunamadı. <Link href="/" className="text-[#6750A4] underline">Anasayfaya dön</Link> — Eski link <code>/anime/{id}</code> yerine <code>/anime?id={id}</code> kullanın</div>;
        ...
        <div className="relative p-6 lg:p-8 flex flex-col lg:flex-row gap-6 items-end">
          ...
          <div className="flex-1 min-w-0 text-white">
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-bold ${meta.status==="Ongoing"?"bg-[#6BCB77] text-black":"bg-white/20 backdrop-blur border border-white/20"}`}>{meta.status==="Ongoing"?"ONGOING":"FINISHED"}</span>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-5 text-sm">
              ...
              <div className="bg-white/10 backdrop-blur rounded-2xl p-3 border border-white/15"><div className="text-white/60 text-xs flex items-center gap-1"><Building2 className="w-3 h-3"/> Stüdyo</div><div className="font-bold truncate">{data.networks?.[0]?.name || "—"}</div></div>
            ...
            <div className="flex flex-wrap gap-2 mt-5">
              ...
              <button onClick={()=> {
                ...
                updateStatus(data.id, entry.status==="WATCHING" && next>=max ? "COMPLETED" : "WATCHING", next);
...
export default function AnimeQueryPage(){
  return (
    <Suspense fallback={<div className="h-[60vh] rounded-[28px] bg-[#F3EDF7] dark:bg-[#211F26] animate-pulse"/>}>
      <AnimePageContent/>
    </Suspense>
  );
}
