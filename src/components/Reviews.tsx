...
export default function Reviews({ animeId }: { animeId: number }) {
  ...
  return (
    <div className="m3-card p-4 lg:p-6">
      <h3 className="font-bold text-lg mb-4 flex items-center gap-2">İncelemeler <span className="text-xs bg-[#EADDFF] dark:bg-[#4F378B] px-2 py-1 rounded-full">1-10 • Firestore canlı</span></h3>
      ...
      <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="Detaylı incelemenizi yazın... (spoiler varsa belirtin) – Firestore'da saklanacak" className="w-full min-h-[110px] rounded-2xl border border-[#E7E0EC] dark:border-[#49454F] bg-[#F3EDF7] dark:bg-[#211F26] p-3 text-sm outline-none" />
      <div className="flex justify-between items-center mt-3">
        ...
        {user ? (
          <button onClick={submit} disabled={loading} className="m3-button disabled:opacity-60">{loading ? "Gönderiliyor..." : "İncelemeyi Gönder • Firestore"}</button>
        ) : (
      ...
      <div className="mt-6 space-y-3">
        {reviews.length === 0 ? (
          <div className="p-4 rounded-2xl bg-[#F3EDF7] dark:bg-[#2B2930] text-sm text-center text-[#49454F]">Henüz inceleme yok — ilk incelemeyi sen yaz! (Gerçek veritabanı)</div>
        ) : reviews.map((r) => (
