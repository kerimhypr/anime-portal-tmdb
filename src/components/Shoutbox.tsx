  ...
  return (
    <div className="m3-card flex flex-col h-[520px]">
      <div className="p-4 flex items-center justify-between border-b border-[#F3EDF7] dark:border-[#2B2930]">
        ...
        <span className="inline-flex items-center gap-1.5 text-xs bg-[#E8DEF8] dark:bg-[#4F378B] px-2.5 py-1 rounded-full"><Users className="w-3.5 h-3.5" /> Firestore canlı</span>
      </div>
      <div ref={ref} className="flex-1 overflow-auto p-3 space-y-2 bg-[#FFFBFE] dark:bg-[#141218]">
        {msgs.length === 0 ? <div className="py-10 text-center text-xs text-[#49454F]">Henüz mesaj yok — ilk mesajı sen gönder! (Firestore'da saklanır)</div> :
          msgs.map((m) => (
      ...
      <div className="p-3 border-t border-[#F3EDF7] dark:border-[#2B2930] flex gap-2">
        <input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder={user ? "Mesaj yaz... (Firestore canlı)" : "Giriş yap ve yaz..."} className="flex-1 h-10 rounded-full bg-[#F3EDF7] dark:bg-[#211F26] px-4 text-sm outline-none border border-transparent focus:border-[#6750A4]" />
        {user ? (
