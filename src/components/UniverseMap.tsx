      ...
      <div className="relative">
        ...
        <div className="grid lg:grid-cols-2 gap-3">
          {nodes.map((n:any,i:number)=>(
            <Link key={n.id} href={n.media_type==="movie" || n.title ? `/movie?id=${n.id}` : `/anime?id=${n.id}`} className="flex gap-3 p-3 rounded-2xl bg-[#F3EDF7] dark:bg-[#211F26] hover:shadow-m3-1 transition border border-transparent hover:border-[#E7E0EC] group">
